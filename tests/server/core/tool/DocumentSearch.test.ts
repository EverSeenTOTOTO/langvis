...
import DocumentSearchTool from '@/server/core/tool/DocumentSearch';
import type { DocumentSearchOutput } from '@/server/core/tool/DocumentSearch/config';
...
async function collectEvents(
  generator: AsyncGenerator<AgentEvent, DocumentSearchOutput, void>,
): Promise<{
  progress: Array<{ message?: string }>;
  result: DocumentSearchOutput | null;
}> {
  const progress: Array<{ message?: string }> = [];
  let result: DocumentSearchOutput | null = null;
...
describe('DocumentSearchTool', () => {
  let retrieveTool: DocumentSearchTool;
  let mockDb: DatabaseService;
  let mockEmbeddingGenerateTool: EmbeddingGenerateTool;
  ...
  beforeEach(() => {
    ...
    } as unknown as EmbeddingGenerateTool;

    container.register(ToolIds.EMBEDDING_GENERATE, {
      useValue: mockEmbeddingGenerateTool,
    });

    retrieveTool = new DocumentSearchTool(mockDb);
