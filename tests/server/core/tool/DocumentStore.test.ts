import DocumentStoreTool from '@/server/core/tool/DocumentStore';
import type { DocumentStoreOutput } from '@/server/core/tool/DocumentStore/config';
...
async function collectEvents(
  generator: AsyncGenerator<AgentEvent, DocumentStoreOutput, void>,
): Promise<{
  progress: Array<{ message?: string }>;
  result: DocumentStoreOutput | null;
}> {
  const progress: Array<{ message?: string }> = [];
  let result: DocumentStoreOutput | null = null;
...
describe('DocumentStoreTool', () => {
  let archiveTool: DocumentStoreTool;
  ...
  beforeEach(() => {
    ...
    archiveTool = new DocumentStoreTool(mockDb);
