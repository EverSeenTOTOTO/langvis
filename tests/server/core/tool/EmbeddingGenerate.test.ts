import EmbeddingGenerateTool from '@/server/core/tool/EmbeddingGenerate';
import type { LlmService } from '@/server/service/LlmService';
import type { EmbeddingGenerateOutput } from '@/server/core/tool/EmbeddingGenerate/config';
...
async function collectEvents(
  generator: AsyncGenerator<AgentEvent, EmbeddingGenerateOutput, void>,
): Promise<{
  progress: Array<{ message?: string }>;
  result: EmbeddingGenerateOutput | null;
}> {
  const progress: Array<{ message?: string }> = [];
  let result: EmbeddingGenerateOutput | null = null;
...
describe('EmbeddingGenerateTool', () => {
  let embedTool: EmbeddingGenerateTool;
  let mockLlmService: LlmService;

  beforeEach(() => {
    vi.clearAllMocks();
    mockLlmService = createMockLlmService();
    embedTool = new EmbeddingGenerateTool(mockLlmService);
    (embedTool as any).logger = logger;
  });

  describe('call', () => {
    it('should generate embeddings for chunks', async () => {
      const mockEmbeddingGenerateding = [0.1, 0.2, 0.3];
      vi.mocked(mockLlmService.embed).mockResolvedValue([
        { embedding: mockEmbeddingGenerateding },
      ]);
      ...
      expect(result!.chunks[0].embedding).toEqual(mockEmbeddingGenerateding);
      expect(result!.dimension).toBe(3);
    });

    it('should use custom model when specified', async () => {
      const mockEmbeddingGenerateding = [0.1, 0.2];
      vi.mocked(mockLlmService.embed).mockResolvedValue([
        { embedding: mockEmbeddingGenerateding },
      ]);
    ...
    it('should handle API failure from LlmService', async () => {
      vi.mocked(mockLlmService.embed).mockRejectedValue(
        new Error(
          'EmbeddingGenerateding API failed: 500 - Internal Server Error',
        ),
      );
      ...
      ).rejects.toThrow('EmbeddingGenerateding API failed: 500');
    });
