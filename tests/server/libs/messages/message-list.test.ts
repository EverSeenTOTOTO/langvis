...
describe('MessageList (monad)', () => {
  it('of 构造并隔离外部数组 mutation', () => {
    const src = [1, 2, 3];
    const list = MessageList.of(src);
    src.push(4);
    expect(list.toArray()).toEqual([1, 2, 3]);
  });

  it('length / get / toArray', () => {
    const list = MessageList.of(['a', 'b']);
  ...
  it('map 返回新 monad、不改原', () => {
    const list = MessageList.of([1, 2, 3]);
  ...
  it('flatMap（bind）摊平', () => {
    const result = MessageList.of([1, 2, 3]).flatMap(x =>
      MessageList.of([x, x]),
    );
  ...
  it('fold 离开 monad 归约', () => {
    const sum = MessageList.of([1, 2, 3, 4]).fold(0, (acc, x) => acc + x);
    expect(sum).toBe(10);
    const indexed = MessageList.of(['a', 'b']).fold(
      [] as string[],
      (acc, m, i) => [...acc, `${i}:${m}`],
    );
  ...
  it('take / drop / takeLast / dropLast / slice', () => {
    const list = MessageList.of([1, 2, 3, 4, 5]);
  ...
  it('takeLast/dropLast 对越界安全', () => {
    expect(MessageList.of([1, 2]).takeLast(10).toArray()).toEqual([1, 2]);
    expect(MessageList.of([1, 2]).dropLast(10).toArray()).toEqual([]);
  });

  it('concat / append / prepend / insertAt', () => {
    const a = MessageList.of([1, 2]);
    expect(a.concat(MessageList.of([3, 4])).toArray()).toEqual([1, 2, 3, 4]);
  ...
  it('链式组合：压缩 transform 形态', () => {
    ...
    const list = MessageList.of(['seed', 'a1', 'a2', 'a3', 'a4', 'a5']);
    const base = 1;
    const keepRecent = 2;
