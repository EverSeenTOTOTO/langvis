// eval harness 域无关类型。不含任何具体业务域（flight/safety/…）形状—— 域把自家沙箱形状塞进 Task&lt;S&gt; 的 S，runner 不感知 S 内部。
...
// 单次 parse 失败的归类，由 react-loop 回灌的 error.message + 模型坏输出原文共同判定。
// split-object：thought 与 tool/input 裂成两对象；missing-tool/missing-input：缺字段；其余=JSON 坏掉。
...
export interface Task<S = unknown> {
  ...
  userGoal: string;
  // 构造本任务（本域）的 per-run 沙箱 + 虚构工具定义 + ToolSet。
  // 工具类是无状态 singleton，沙箱经 runId 绑定，故 setup 每次返回 fresh 沙箱。
  setup(): { sandbox: S; tools: FictionalToolDef[]; toolSet: ToolSet };
  // 可选：workDir 注入后、run 前预置文件（如拷 PDF）。setup 先于 workDir 返回，
  // 故文件预置须在此钩子（runner 在 attachWorkDir 后调用）。域无关。
...
// 多 turn 任务：user 消息逐轮驱动，轮间跑完整 conv 变换链（summary-attach/compact），
// 跨轮共享沙箱，让四条压缩机制都可观测；success 拿末轮 run + 全 turn 合并 events。
export interface MultiTurnTask<S = unknown> extends Omit<Task<S>, 'userGoal'> {
  readonly turns: string[];
  /** 可选：system 之后、turns 之前的会话历史（坏 demo 测抗投毒 / 审计反幻觉）。runner 填 id 等字段。 */
