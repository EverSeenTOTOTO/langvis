...
if (ctx.depth >= MAX_DEPTH) {
  throw new Error(`Agent call depth exceeded: ${ctx.depth}`);
}
```
...
async *call(input: AgentCallInput, ctx: ExecutionContext) {
  ...
  // yield 开始状态
  yield ctx.agentToolProgressEvent(this.id, { status: 'calling', agentId });
  ...
  try {
    ...
    throw error; // 让 Agent 处理错误，生成 tool_error 事件
  }
}
```
